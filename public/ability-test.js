...
// 문제 데이터 받아오기 (서버에서 전달하는 데이터를 여기에 대입)
const words = []; // Words 모델에서 받아온 데이터를 저장하는 배열
let wrongChoices = []; // 오답 선지를 저장하는 배열

// 서버에서 Words 데이터와 오답 선지 데이터 받아오기
Promise.all([fetch('/words2').then((response) => response.json()), fetch('/wrong-choices').then((response) => response.json())])
  .then((data) => {
    words.push(...data[0]);
    wrongChoices = data[1];
    generateQuestion();
...
function generateQuestion() {
  ...
  // 선택지 배열에 오답 선택지 추가
  choices.push(wrongChoices[0]);
  choices.push(wrongChoices[1]);
  choices.push(wrongChoices[2]);
